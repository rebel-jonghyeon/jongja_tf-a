...
static uint32_t get_uart_burdrate_divisor(const struct device *dev, uint32_t baud_rate,
										  uint32_t pclk)
{
	ARG_UNUSED(dev);
	/*
...
#ifdef CONFIG_UART_NS16550_ITE_HIGH_SPEED_BUADRATE
static uint32_t get_ite_uart_burdrate_divisor(const struct device *dev, uint32_t baud_rate,
											  uint32_t pclk)
{
...
static int uart_ns16550_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	ns16550_outbyte(dev_cfg, LCR(dev), uart_cfg.data_bits | uart_cfg.stop_bits | uart_cfg.parity);
...
static int uart_ns16550_init(const struct device *dev)
{
...
#if defined(CONFIG_UART_ASYNC_API)
		if (data->async.tx_dma_params.dma_dev) {
	...
	if (data->async.tx_dma_params.dma_dev) {
		...
		data->async.rx_dma_params.dma_cfg.head_block = &data->async.rx_dma_params.active_dma_block;
		data->async.tx_dma_params.dma_cfg.head_block = &data->async.tx_dma_params.active_dma_block;
#if defined(CONFIG_UART_NS16550_INTEL_LPSS_DMA)
#if UART_NS16550_IOPORT_ENABLED
		if (!dev_cfg->io_map)
#endif
...
#if (IS_ENABLED(CONFIG_UART_ASYNC_API))
	if (dev_data->async.tx_dma_params.dma_dev) {
...
static void dma_callback(const struct device *dev, void *user_data, uint32_t channel, int status)
{
	...
	if (channel == tx_params->dma_channel) {
		...
		if (rx_params->buf && rx_params->buf_len > 0) {
			dma_reload(dev, rx_params->dma_channel, data->phys_addr, (uintptr_t)rx_params->buf,
					   rx_params->buf_len);
...
static int uart_ns16550_rx_buf_rsp(const struct device *dev, uint8_t *buf, size_t len)
{
	...
	assert(!data->async.next_rx_buffer);

...
#define HCH_ENABLE				1
#define HCH_DISABLE				0

#define SET_CP0_ADD_HCH(val)							\
	do {												\
		uint32_t *en_add = SYSREG_CP0_PRIVATE + 0x378;	\
		*en_add = (val);								\
	} while (0)

#define SET_CP0_TEMP_HCH(val)							\
	do {												\
		uint32_t *en_temp = SYSREG_CP0_PRIVATE + 0x37c;	\
		*en_temp = (val);								\
	} while (0)

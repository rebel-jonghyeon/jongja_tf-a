...
enum {
	PERF_KEY_HC_OPEN = 0,
	PERF_KEY_HC_CLOSE = 1,
	PERF_KEY_XMSS_VERIFY = 2,
	PERF_KEY_XMSS_MT_VERIFY = 3,
};
...
static void print_usage(void)
{
	...
	LOG_INF("COMMAND:  open|close|xmss-verify|xmss-mt-verify\n");
...
static int process_xmss_verify(uint8_t *argp, uint32_t argc, size_t *_buflen)
{
	int rc = -1;
	...
	LOG_DBG("performing cmrt_xmss_verify with plen=%lu mlen:%lu slen=%lu",
		(unsigned long)plen, (unsigned long)mlen, (unsigned long)slen);
	perf_start();
	rc = cmrt_xmss_verify(hc, pk, msg, mlen, sig, NULL);
	perf_end(PERF_KEY_XMSS_VERIFY);
	LOG_DBG("cmrt_xmss_verify res: %d", rc);
...
static int process_xmss_mt_verify(uint8_t *argp, uint32_t argc, size_t *_buflen)
{
	int rc = -1;
	...
	LOG_DBG("performing cmrt_xmss_mt_verify with plen=%lu mlen:%lu slen=%lu",
		(unsigned long)plen, (unsigned long)mlen, (unsigned long)slen);
	perf_start();
	rc = cmrt_xmss_mt_verify(hc, pk, msg, mlen, sig, NULL);
	perf_end(PERF_KEY_XMSS_MT_VERIFY);
	LOG_DBG("cmrt_xmss_mt_verify res: %d", rc);
...
static int process_message(uint8_t *buffer, size_t *_buflen)
{
	...
	*_buflen = 0;
	...
	if (MATCH(argp, argl, "xmss-verify")) {
		return process_xmss_verify(argp_next, argc, _buflen);
	}
	if (MATCH(argp, argl, "xmss-mt-verify")) {
		return process_xmss_mt_verify(argp_next, argc, _buflen);
	}
...
{
	LOG_INF("Test (XMSS) main loop starting...\n");
#else
int main(void)
{
	LOG_INF("Kernel (XMSS) starting...\n");
	uint32_t flowid = 100;

...
{
#if defined(CONFIG_4TH_LEVEL_INTERRUPTS)
	return ((irq >> 24) & 0xFF) != 0 ? 4 :
		(((irq >> 16) & 0xFF) != 0 ? 3 :
		 (((irq >> 8) & 0xFF) == 0 ? 1 : 2));
#elif defined(CONFIG_3RD_LEVEL_INTERRUPTS)
...
#define IRQ_FROM_LEVEL_2(irq) ((((irq) >> 8) & 0xFF) - 1)
...
#define IRQ_TO_LEVEL_2(irq) (((irq) + 1) << 8)
...
{
	return ((irq >> 16) & 0xFF) - 1;
}
#define IRQ_FROM_LEVEL_3(irq) ((((irq) >> 16) & 0xFF) - 1)
...
#define IRQ_TO_LEVEL_3(irq) (((irq) + 1) << 16)
...
#ifdef CONFIG_4TH_LEVEL_INTERRUPTS
/**
 * @brief Return the 4th level interrupt number
 * @def irq_from_level_4()
 *
 * This routine returns the third level irq number of the zephyr irq
 * number passed in
 *
 * @param irq IRQ number in its zephyr format
 *
 * @return 4th level IRQ number
 */
static inline unsigned int irq_from_level_4(unsigned int irq)
{
	return ((irq >> 24) & 0xFF) - 1;
}
#define IRQ_FROM_LEVEL_4(irq) ((((irq) >> 24) & 0xFF) - 1)

/**
 * @brief Converts irq from level 1 to level 4 format
 * @def irq_to_level_4()
 *
 * This routine converts the input into the level 4 irq number format
 *
 * @note Values >= 0xFF are invalid
 *
 * @param irq IRQ number in its zephyr format
 ...
 * @return 4th level IRQ number
 */
static inline unsigned int irq_to_level_4(unsigned int irq)
{
	return (irq + 1) << 24;
}
#define IRQ_TO_LEVEL_4(irq) (((irq) + 1) << 24)

/**
 * @brief Returns the parent IRQ of the level 4 raw IRQ number
 * @def irq_parent_level_4()
 *
 * The parent of a 4th level interrupt is in the 2nd byte
 *
 * @param irq IRQ number in its zephyr format
 *
 * @return 4th level IRQ parent
 */
static inline unsigned int irq_parent_level_4(unsigned int irq)
{
	return (irq >> 16) & 0xFF;
}
#endif

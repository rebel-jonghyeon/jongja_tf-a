...
typedef struct cmrt_kdc_context *cmrt_kdc_t;
...
typedef struct {
	...
	/** Lengh of diversify_path. must be [1, 16]. */
	uint32_t diversify_len;

	/** The number octets in diversify_path must be within [1, 16]. */
 ...
 *
 * @param flags Zero to call cmrt_kdc_sync after an operation,
 *              CMRT_O_SYNC to autosync all operations.
 * @return Context handle on success or NULL on error.
 */
__syscall cmrt_kdc_t cmrt_kdc_open(uint32_t flags);
 ...
 */
__syscall void cmrt_kdc_close(cmrt_kdc_t context);

/**
 * @brief Burn DGOK to OTP.
 *
 * Burn DGOK to OTP. Caller must call cmrt_kdc_sync to wait for completion.
 * This can be done only once and is typically executed at device
 * personalization in machine mode. Caller must call cmrt_kdc_sync,
 * or CMRT_O_SYNC must be set in cmrt_kdc_open.
 *
 * @note The driver stores a reference to the @p cmd parameter that is
 * used in cmrt_kdc_sync(). Callers must ensure that the parameter
 * value lives at least until cmrt_kdc_sync() is called.
 *
 * @param context Pointer to the device.
 ...
 * @brief Derive key.
 *
 * Derive key. Caller must call cmrt_kdc_sync to wait for completion.
 * Key will be derived based on the previously set derive path and
 * diversify path within the cmd. Caller must call cmrt_kdc_sync,
 * or CMRT_O_SYNC must be set in cmrt_kdc_open.
 *
 * The derive operation uses the first HC and possibly OMC internally.
 * EBUSY is returned if reservation for these HW cores fails.
 *
 * @note The driver stores a reference to the @p cmd parameter that is
 * used in cmrt_kdc_sync(). Callers must ensure that the parameter
 * value lives at least until cmrt_kdc_sync() is called.
 ...
 * @return Zero on success, -ERRNO on error.
 */
__syscall int cmrt_kdc_derive_key(cmrt_kdc_t context, cmrt_kdc_cmd_t *cmd);

/**
 * @brief Diversify key.
 *
 * Diversify key. Can be called after cmrt_kdc_derive_key in case full
 ...
 * Caller must call cmrt_kdc_sync, or CMRT_O_SYNC must be set in cmrt_kdc_open.
 *
 * The diversify operation uses the first HC and possibly OMC internally.
 * EBUSY is returned if reservation for these HW cores fails.
 *
 * @note The driver stores a reference to the @p cmd parameter that is
 * used in cmrt_kdc_sync(). Callers must ensure that the parameter
 ...
 * @return Zero on success, -ERRNO on error.
 */
__syscall int cmrt_kdc_diversify_key(cmrt_kdc_t context, cmrt_kdc_cmd_t *cmd);
 ...
 * @return Zero on success, -ERRNO on error.
 */
__syscall int cmrt_kdc_sync(cmrt_kdc_t context);

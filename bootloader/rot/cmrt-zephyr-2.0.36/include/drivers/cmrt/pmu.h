 ...
 */
__syscall int cmrt_pmu_low_power(bool allowed);

/**
 * @brief Set idle mode from where HLOS can command low power mode.
 *
 * Typically called from the idle thread after cmrt_pmu_low_power(true) has
 ...
 * cmrt_pmu_to_idle() the application thread must execute the WFI instruction

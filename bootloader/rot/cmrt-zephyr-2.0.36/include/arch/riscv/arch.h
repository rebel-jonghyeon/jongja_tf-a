...
#if defined(CONFIG_USERSPACE) || defined(CONFIG_SOC_SERIES_RISCV_CMRT)
...
/* stacks, for RV32E architecture stack should be 4byte-aligned */
...
#define MSTATUS_IEN     (1UL << 1)
#define MSTATUS_MPP_M   (1UL << 8)
#define MSTATUS_MPIE_EN (1UL << 5)
#endif
...
#if defined(CONFIG_RISCV_HAS_PLIC) || defined(CONFIG_RISCV_HAS_SLIC)
...
#ifndef CONFIG_SUPERVISOR_MODE
	mstatus = csr_read_clear(mstatus, MSTATUS_IEN);
#else
	mstatus = csr_read_clear(sstatus, MSTATUS_IEN);
#endif
...
#ifndef CONFIG_SUPERVISOR_MODE
	mstatus = csr_read_set(mstatus, key & MSTATUS_IEN);
#else
	mstatus = csr_read_set(sstatus, key & MSTATUS_IEN);
#endif
}
...
{
#if defined(CONFIG_JTAG_BEFORE_BOOT) && !defined(CONFIG_BOOTBYPASS_BEFORE_BOOT)
	/* WFI would confuse the debugger. */
	__asm__ volatile("nop");
#else
...
static ALWAYS_INLINE void arch_ecall(void)
{

...
FORTH_LVL_INTERRUPTS = 0xFF000000
...
def write_source_file(fp, vt, swt, intlist, syms):
    ...
    level4_offset = syms.get("CONFIG_4TH_LVL_ISR_TBL_OFFSET")
    ...
    for i in range(nv):
        ...
        if level4_offset is not None and i == level4_offset:
            fp.write("\t/* Level 4 interrupts start here (offset: {}) */\n".
                     format(level4_offset))
    ...
    if "CONFIG_MULTI_LEVEL_INTERRUPTS" in syms:
        ...
        if "CONFIG_2ND_LEVEL_INTERRUPTS" in syms:
            ...
            if "CONFIG_3RD_LEVEL_INTERRUPTS" in syms:
                ...
                if "CONFIG_4TH_LEVEL_INTERRUPTS" in syms:
                    num_aggregators = syms["CONFIG_NUM_4TH_LEVEL_AGGREGATORS"]
                    irq4_baseoffset = syms["CONFIG_4TH_LVL_ISR_TBL_OFFSET"]
                    list_4th_lvl_offsets = [syms['CONFIG_4TH_LVL_INTR_{}_OFFSET'.
                                                 format(str(i).zfill(2))] for i in
                                            range(num_aggregators)]

                    debug('4th level offsets: {}'.format(list_4th_lvl_offsets))
    ...
    for irq, flags, func, param in intlist["interrupts"]:
        if flags & ISR_FLAG_DIRECT:
            ...
            if not "CONFIG_MULTI_LEVEL_INTERRUPTS" in syms:
                ...
                # Figure out fourth level interrupt position
                debug('IRQ = ' + hex(irq))
                irq4 = (irq & FORTH_LVL_INTERRUPTS) >> 24
                ...
                if irq4:
                    irq_parent = irq3
                    list_index = getindex(irq_parent, list_4th_lvl_offsets)
                    irq4_pos = irq4_baseoffset + max_irq_per*list_index + irq4 - 1
                    debug('IRQ_level = 4')
                    debug('IRQ_Indx = ' + str(irq4))
                    debug('IRQ_Pos  = ' + str(irq4_pos))
                    table_index = irq4_pos - offset

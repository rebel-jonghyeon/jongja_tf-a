...
#define CAT_4TH_LVL_LIST(i, base) \
	INIT_IRQ_PARENT_OFFSET(CONFIG_4TH_LVL_INTR_0##i##_OFFSET, \
		IRQ_INDEX_TO_OFFSET(i, base))
static struct irq_parent_offset lvl4_irq_list[CONFIG_NUM_4TH_LEVEL_AGGREGATORS]
	 = { UTIL_LISTIFY(CONFIG_NUM_4TH_LEVEL_AGGREGATORS, CAT_4TH_LVL_LIST,
		CONFIG_4TH_LVL_ISR_TBL_OFFSET) };

#endif /* CONFIG_4TH_LEVEL_INTERRUPTS */
		   ...
		   const void *param)
{
	...
	if (level == 2U) {
		...
		parent_irq = irq_parent_level_4(irq);
		parent_offset = get_parent_offset(parent_irq,
			lvl4_irq_list,
			CONFIG_NUM_4TH_LEVEL_AGGREGATORS);
		table_idx = parent_offset + irq_from_level_4(irq);
	}
#endif /* CONFIG_4TH_LEVEL_INTERRUPTS */
	else {
		table_idx = irq;

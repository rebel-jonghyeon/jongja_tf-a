...
struct __esf *idle_stack_init;
#include <drivers/cmrt/sic_reg.h>
extern void *fboot_bootbypass_vector(uint32_t state);
#endif
		     ...
		     void *p1, void *p2, void *p3)
{
...
#ifdef CONFIG_CMRT_PMU
	if (entry == idle) {
		/* Save idle thread initial stack for boot bypass. */
		idle_stack_init = stack_init;
		/* Keep the initial stack frame untouched for wakeup. */
		stack_ptr = (char *)idle_stack_init;
	}
#endif
	/* Setup the initial stack frame */
#ifdef CONFIG_SOC_SERIES_RISCV_CMRT
	stack_init->sp = (ulong_t)stack_ptr;
#endif
	...
	/* Set global pointer to supervisor gp. */
	extern uint32_t __global_pointer$;
	stack_init->gp = (ulong_t)&__global_pointer$;
#ifdef CONFIG_USERSPACE
	/* User mode thread may have different gp, use the same value now. */
	thread->arch.user_gp = (ulong_t)&__global_pointer$;
#endif
#endif
	...
	csr_write(0x7c2, (ulong_t)stack_ptr);
#else
	csr_write(0x9c2, (ulong_t)stack_ptr);
#endif
#endif
}
	...
	void *p1, void *p2, void *p3)
{
...
#endif
	status = INSERT_FIELD(status, MSTATUS_MPP_M, PRV_U);
	status = INSERT_FIELD(status, MSTATUS_MPRV, 0);

	bool is_container = (_current->base.user_options & K_CONTAINER) != 0;
	ulong_t entry = is_container ? (ulong_t)user_entry : (ulong_t)z_thread_entry_wrapper;
#ifndef CONFIG_SUPERVISOR_MODE
	...
	__asm__ volatile ("mv sp, %1"
			  ...
			  : "memory");
#ifdef CONFIG_RISCV_GP
	__asm__ volatile ("mv gp, %1"
			  : "=r" (_current->arch.user_gp)
			  : "r" (_current->arch.user_gp)
			  : "memory");
#endif
...
#ifdef CONFIG_CMRT_CFI
	/* Initialize shadow stack frame pointer. */
	__asm__ volatile ("csrw 0x7c2, sp");
#endif
	__asm__ volatile ("mret");
#else
#ifdef CONFIG_CMRT_CFI
	/* Initialize shadow stack frame pointer. */
	__asm__ volatile ("csrw 0x9c2, sp");
#endif
...
FUNC_NORETURN void z_riscv_switch_to_main_no_multithreading(
	k_thread_entry_t main_entry, void *p1, void *p2, void *p3)
{
	ulong_t main_stack = (ulong_t)z_main_stack + CONFIG_MAIN_STACK_SIZE;
	...
	/* Enable interrupts for the main thread. */
#ifndef CONFIG_SUPERVISOR_MODE
#if defined(CONFIG_CMRT_FBOOT) && defined(CONFIG_CMRT_PMU)
	/* Only enable if not in boot bypass. */
	uint32_t state = sys_read32((mm_reg_t)(CMRT_SIC_BASE + R_DEVICE_LIFECYCLE));
	if (fboot_bootbypass_vector(state) == NULL)
#endif
	csr_write(mstatus, MSTATUS_IEN);
#else
	csr_write(sstatus, MSTATUS_IEN);
#endif
	(*main_entry)(p1, p2, p3);

	arch_system_halt(ECHILD);
}
#endif

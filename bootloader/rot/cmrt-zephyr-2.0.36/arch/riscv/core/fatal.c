...
void _Fault(z_arch_esf_t *esf)
{
	...
	if (mcause == RISCV_LOAD_ACCESS_EXP) {
		/* Handle OTP ECC faults. */
		extern bool otp_access_fixup(z_arch_esf_t *esf);
		if (otp_access_fixup(esf)) {
			/* Handled, return execution. */
			return;
		}
	}
#endif
	if (false
	    ...
	    || mcause == RISCV_LOAD_MISALIGNED_EXP
	    || mcause == RISCV_STORE_MISALIGNED_EXP
#endif
#ifdef CONFIG_CMRT_SAC_ACCESS_FIXUP
	    || mcause == RISCV_LOAD_ACCESS_EXP
	    || mcause == RISCV_STORE_ACCESS_EXP
#endif
	    ) {
		/* Handle misaligned/SAC access faults. */
		extern bool cmrt_access_fixup(z_arch_esf_t *esf);
		if (cmrt_access_fixup(esf)) {
			/* Handled, return execution. */
			return;
		}
	}
...
{
	_current->syscall_frame = ssf_ptr;

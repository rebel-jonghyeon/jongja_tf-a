...
/* device config */
struct uart_device_mmio_rom {
	/** MMIO linear address */
	uint64_t addr;
};
#define UART_DEVICE_MMIO_ROM     struct uart_device_mmio_rom _mmio
struct uart_ns16550_device_config {
#ifndef UART_NS16550_ACCESS_IOPORT
	UART_DEVICE_MMIO_ROM;
#else
	uint32_t port;
#endif
	uint64_t base;
...
{
	return CMRT_SAC_BASE + R_CPU_DATA;
}
...
{
	int ret;
	uint64_t old_base = cmrt_sac_get_base(NULL);

	DEV_DATA(dev)->chipelt_offset = get_chiplet_id() * CHIPLET_BASE_OFFSET;
	/* set sac to uart base addr */
	cmrt_sac_set_base(NULL, DEV_CFG(dev)->base + DEV_DATA(dev)->chipelt_offset);
	ret = uart_ns16550_configure(dev, &DEV_DATA(dev)->uart_config);
	if (ret != 0) {
		cmrt_sac_set_base(NULL, old_base);
		return ret;
	}
...
#ifdef CONFIG_UART_NS16550_NOP_MODE
	/* suppress output in nop mode */
	if ((INBYTE(MDC(dev)) & MCR_LOOP) == 0)
#endif
	OUTBYTE(THR(dev), c);

...
#define REBEL_40BIT_HIGH_ADDR_MASK      (0xff00000000ULL)
...
static void dma_pl330_cfg_dmac_add_control(uint64_t control_reg_base,
										   uint64_t dst, uint64_t src, int ch)
{
	cmrt_sac_set_base(NULL, SYSREG_SYSREMAP_BASE);
	struct sysreg_sysremap_regs *control =
		(struct sysreg_sysremap_regs *)(CMRT_SAC_BASE + R_CPU_DATA);

	control->m3_prot_ovrd_value.val = 0;
...
static inline uint32_t dma_pl330_gen_mov(uint64_t buf,
										 enum dmamov_type type,
										 uint32_t val)
{
	cmrt_sac_set_base(NULL, buf & CPU_ADDR_MASK);
	uint32_t buf_offset = buf & ~CPU_ADDR_MASK;

	cmrt_sac_write8(OP_DMA_MOV, buf_offset + SAC_MEMORY_BASE);
	cmrt_sac_write8(type, buf_offset + 1 + SAC_MEMORY_BASE);
	cmrt_sac_write8(val, buf_offset + 2 + SAC_MEMORY_BASE);
	cmrt_sac_write8(val >> 8, buf_offset + 3 + SAC_MEMORY_BASE);
	cmrt_sac_write8(val >> 16, buf_offset + 4 + SAC_MEMORY_BASE);
	cmrt_sac_write8(val >> 24, buf_offset + 5 + SAC_MEMORY_BASE);
...
{
	cmrt_sac_set_base(NULL, addr & CPU_ADDR_MASK);
	uint32_t addr_offset = addr & ~CPU_ADDR_MASK;

	cmrt_sac_write8(opcode, addr_offset + SAC_MEMORY_BASE);
	cmrt_sac_write8(val, addr_offset + 1 + SAC_MEMORY_BASE);
}
							  ...
							  int ch)
{
	...
	uint32_t dma_exec_addr_offset = dma_exec_addr & ~CPU_ADDR_MASK;
	...
	if (loop_counter >= PL330_LOOP_COUNTER0_MAX) {
		...
		dma_pl330_gen_op(OP_DMA_LOOP, dma_exec_addr + offset,
						 loop_counter0 & BYTE_MASK);
		offset = offset + 2;
		lp1_start = offset;
		lp0_start = offset;

		cmrt_sac_set_base(NULL, dma_exec_addr & CPU_ADDR_MASK);
		cmrt_sac_write8(OP_DMA_LD, dma_exec_addr_offset + offset + SAC_MEMORY_BASE);
		cmrt_sac_write8(OP_DMA_ST, dma_exec_addr_offset + offset + 1 + SAC_MEMORY_BASE);
	...
	cmrt_sac_set_base(NULL, dma_exec_addr & CPU_ADDR_MASK);
	cmrt_sac_write8(OP_DMA_END, dma_exec_addr_offset + offset + SAC_MEMORY_BASE);
	cmrt_sac_write8(OP_DMA_END, dma_exec_addr_offset + offset + 1 + SAC_MEMORY_BASE);
	cmrt_sac_write8(OP_DMA_END, dma_exec_addr_offset + offset + 2 + SAC_MEMORY_BASE);
	cmrt_sac_write8(OP_DMA_END, dma_exec_addr_offset + offset + 3 + SAC_MEMORY_BASE);

	return 0;
}

static int dma_pl330_start_dma_ch(const struct device *dev,
								  uint64_t reg_base, int ch, int secure)
{
	struct dma_pl330_dev_data *const dev_data = DEV_DATA(dev);
	struct dma_pl330_ch_config *channel_cfg;
	uint32_t count = 0U;
	...
	channel_cfg = &dev_data->channels[ch];
	cmrt_sac_set_base(NULL, reg_base);
	do {
		data = cmrt_sac_read32(DMAC_PL330_DBGSTATUS + SAC_MEMORY_BASE);
	...
	cmrt_sac_set_base(NULL, reg_base);
	cmrt_sac_write32(((ch << DMA_INTSR1_SHIFT) +
					  (DMA_INTSR0 << DMA_INTSR0_SHIFT) +
					  (secure << DMA_SECURE_SHIFT) + (ch << DMA_CH_SHIFT)),
					 DMAC_PL330_DBGINST0 + SAC_MEMORY_BASE);

	cmrt_sac_write32(channel_cfg->dma_exec_addr, DMAC_PL330_DBGINST1 + SAC_MEMORY_BASE);
	cmrt_sac_write32(0x0, DMAC_PL330_DBGCMD + SAC_MEMORY_BASE);

	count = 0U;
	do {
		data = cmrt_sac_read32(DMAC_PL330_DBGCMD + SAC_MEMORY_BASE);
		if (++count > DMA_TIMEOUT_US) {
			return -ETIMEDOUT;
		}
		k_busy_wait(1);
...
static int dma_pl330_wait(uint64_t reg_base, int ch)
{
	int count = 0U;
	...
	do {
		...
		k_busy_wait(1);
	} while (((cmrt_sac_read32((DMAC_PL330_CS0 + ch * 8) + SAC_MEMORY_BASE)) &
			  CH_STATUS_MASK) != 0);
						  ...
						  uint32_t *xfer_size)
{
	...
	dma_pl330_cfg_dmac_add_control(dev_cfg->reg_base, dst, src, channel);
#endif
	...
	ret = dma_pl330_start_dma_ch(dev, dev_cfg->reg_base, channel, ch_handle->nonsec_mode);
	if (ret) {
									...
									uint32_t channel)
{
	...
	dma_pl330_cfg_dmac_add_control(dev_cfg->reg_base, channel_cfg->dst_addr,
...
static int dma_pl330_initialize(const struct device *dev)
{
	...
	for (int channel = 0; channel < MAX_DMA_CHANNELS; channel++) {
		channel_cfg = &dev_data->channels[channel];
		channel_cfg->dma_exec_addr = dev_cfg->mcode_base + (channel * MICROCODE_SIZE_MAX);
...
#endif
	.mcode_base = (((uint64_t)DT_INST_PROP_BY_IDX(0, microcode, 0)) << 32) | DT_INST_PROP_BY_IDX(0, microcode, 1),
};

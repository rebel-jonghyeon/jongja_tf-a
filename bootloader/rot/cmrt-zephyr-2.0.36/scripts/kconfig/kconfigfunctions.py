...
#pylint: disable=unused-argument,inconsistent-return-statements,import-error,wrong-import-position,redefined-builtin
import os
import pickle
import sys
...
def dt_node_has_label_prop(kconf, _, path, prop):
    """
    This function takes a 'path' and looks for an EDT node at that path. If it
    finds an EDT node, it will look to see if that node has the label property
    by the name of 'prop'.  If 'prop' matches it will return "y" otherwise
    we return "n".
    """
    if doc_mode or edt is None:
        return "n"
...
functions = {
    ...
    "dt_node_has_label_prop": (dt_node_has_label_prop, 2, 2),
...
#pylint: enable=unused-argument,inconsistent-return-statements,import-error,wrong-import-position,redefined-builtin
